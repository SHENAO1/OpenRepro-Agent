...
Current version: **v1.56.0**. This is still an alpha engineering scaffold, not a finished autonomous paper-reproduction system.
...
- Add `openrepro cache add/list/verify/gc`.
- Store observed project artifacts in `.openrepro/cache/sha256/<prefix>/<sha256>`.
- Write `workspace/artifact_cache.json`, `workspace/ARTIFACT_CACHE.md`, `workspace/artifact_cache_validation.json`, and `workspace/ARTIFACT_CACHE_VALIDATION.md`.
...
- Add `openrepro eval define/run`.
- Add `openrepro experiments leaderboard`.
- Write `workspace/evaluation_registry.json`, `workspace/EVALUATION_REGISTRY.md`, `workspace/evaluation_results.json`, and `workspace/EVALUATION_RESULTS.md`.
- Write `workspace/experiment_leaderboard.json` and `workspace/EXPERIMENT_LEADERBOARD.md`.
- Evaluate latest tracked experiment metrics against declared thresholds without claiming scientific reproduction success.

## What v1.43.0 adds

- Add `openrepro agent run`.
- Write `workspace/agent_sandbox_run.json`, `workspace/AGENT_SANDBOX_RUN.md`, and append `workspace/agent_sandbox_trajectory.jsonl`.
- Capture per-step sandbox logs under `workspace/agent_sandbox/<run_id>/`.
- Require both `--confirm` and `--approve` before executing safe allowlisted agent steps.
- Keep experiments, repair apply, human decisions, and claim signoffs blocked.

## What v1.44.0 adds

- Add `openrepro ci init/validate/summary`.
- Write `.github/workflows/openrepro-ci.yml`.
...
- Add `openrepro assets plan/materialize/summary`.
- Write `workspace/asset_build_plan.json`, `workspace/ASSET_BUILD_PLAN.md`, `workspace/asset_materialization.json`, and `workspace/ASSET_MATERIALIZATION.md`.
- Identify safe runnable workflow steps with missing outputs and turn them into incremental materialization candidates.
- Add `local_ui` to the workflow DAG, delivery preset, and refresh pipeline.
- Keep materialization dry-run by default and delegate confirmed execution to the guarded workflow executor.

## What v1.47.0 adds

- Add `openrepro plugins register/list/validate/summary`.
- Write `openrepro.plugins.yaml`, `workspace/plugin_registry.json`, `workspace/PLUGIN_REGISTRY.md`, `workspace/plugin_validation.json`, and `workspace/PLUGIN_VALIDATION.md`.
- Support declarative command, provider, reporter, and evaluator extension entries.
- Validate plugin ids, kinds, run modes, entrypoints, and unsafe `safe_command` declarations.
- Keep plugins declarative: registry generation and validation never import or execute plugin code.

## What v1.48.0 adds

- Add `openrepro promote plan/record/summary`.
- Write `workspace/promotion_plan.json`, `workspace/PROMOTION_PLAN.md`, `workspace/promotion_record.json`, `workspace/PROMOTION_RECORD.md`, `workspace/promotion_registry.json`, and `workspace/PROMOTION_REGISTRY.md`.
- Evaluate promotion gates for `experiment`, `report`, and `delivery` targets.
- Require `--confirm` before a passing promotion plan is recorded.
- Keep promotion gates as engineering release-state controls, not scientific validation.

## What v1.49.0 adds

- Add `openrepro github pr-summary/summary`.
- Write `workspace/github_pr_summary.json`, `workspace/GITHUB_PR_SUMMARY.md`, and `reports/pr_comment.md`.
- Generate a local PR comment draft from CI, local UI, asset build, plugin, promotion, dashboard, and evidence freshness summaries.
...
- Add `openrepro integrations export/summary`.
- Write declaration-only adapter artifacts under `integrations/` for MLflow, Aim, DVC, and Hydra.
...
- Add `openrepro integrations run`.
- Generate supervised adapter scripts for MLflow, Aim, and Hydra plus a DVC dependency preflight.
- Write `workspace/integration_execution.json` and `workspace/INTEGRATION_EXECUTION.md`.
...
- Add `openrepro cockpit build/summary`.
- Write `reports/cockpit/index.html`, `reports/cockpit_manifest.json`, `workspace/cockpit_summary.json`, `workspace/COCKPIT_SUMMARY.md`, and optional `reports/cockpit.zip`.
- Summarize readiness, gaps, data quality, dataset card, claim trace, runs, Bench Lite, integrations, evidence package freshness, and review decisions.
- Generate ordered next actions with suggested commands.
- Surface Cockpit links in evidence packages, dashboards, and readiness reviews.
...
# Replace correlation_peak with a metric observed in experiment tracking.
openrepro eval define boc_demo --metric correlation_peak --threshold 0.8 --operator ">="
openrepro eval run boc_demo
openrepro experiments leaderboard boc_demo --metric correlation_peak
...
openrepro cockpit build boc_demo --zip
...
### `openrepro ci init <project_name> [--test-command COMMAND] [--overwrite]`

Generates a local GitHub Actions workflow scaffold and writes:
...
The workflow installs the package, runs `openrepro --version`, and executes the
configured pytest command.

### `openrepro ci validate <project_name>`

Validates the local CI workflow scaffold and writes:
...
Validation checks local YAML contents only; it does not inspect remote GitHub
Actions runs.

### `openrepro ci summary <project_name>`
...
### `openrepro cockpit build <project_name> [--zip]`

Builds the static reproduction cockpit and writes:

```text
reports/cockpit/index.html
reports/cockpit_manifest.json
workspace/cockpit_summary.json
workspace/COCKPIT_SUMMARY.md
reports/cockpit.zip
```

The cockpit summarizes readiness, data quality, dataset card status, claim
trace validation, latest runs, quality gates, Bench Lite, integration execution,
freshness, review decisions, and ordered next actions. It is an operator review
surface only; it does not prove scientific reproduction.

### `openrepro cockpit summary <project_name>`

Shows the existing cockpit status without mutating project files.
...
### `openrepro agent run <project_name> [--role ROLE] [--confirm] [--approve] [--max-steps N]`

Runs or dry-runs approved safe agent execution-plan steps and writes:

```text
workspace/agent_sandbox_run.json
workspace/AGENT_SANDBOX_RUN.md
workspace/agent_sandbox_trajectory.jsonl
workspace/agent_sandbox/<run_id>/
```

Without `--confirm --approve`, it records a dry run or approval block. With both
flags, it can execute only allowlisted safe derived-artifact commands from
`agent_exec_plan.json`.
...
### `openrepro eval define <project_name> --metric METRIC --threshold VALUE [--operator OP] [--suite NAME]`

Defines or updates an experiment evaluation suite and writes:
...
Evaluation criteria compare observed latest metrics from experiment tracking
against declared thresholds.

### `openrepro eval run <project_name> [--suite NAME]`

Runs an evaluation suite and writes:
...
The result reports passed, failed, and missing metric counts. It is workflow
metric evidence only, not a paper reproduction claim.

### `openrepro experiments leaderboard <project_name> [--suite NAME] [--metric METRIC]`

Ranks tracked experiments by a declared metric and writes:
...
Leaderboards rank observed latest metrics only.

### `openrepro plugins register <project_name> --id ID --kind KIND --entrypoint ENTRYPOINT`

Registers a declarative plugin entry and writes:
...
Kinds are `command`, `provider`, `reporter`, and `evaluator`. Run modes are
`declaration`, `external_supervised`, and `safe_command`.

### `openrepro plugins list <project_name>`

Builds and lists the project plugin registry from `openrepro.plugins.yaml`.

### `openrepro plugins validate <project_name>`

Validates plugin declarations without importing or executing plugin code. It
blocks unsafe command names such as experiment execution, repair apply, and
human decision commands for `safe_command` plugins.

### `openrepro plugins summary <project_name>`

Shows the current plugin registry and validation summary.

### `openrepro integrations export <project_name> [--target TARGET]`

Writes supervised adapter artifacts for MLflow, Aim, DVC, and Hydra under:
...
Exports are declaration-only handoff files. They do not import or execute
external tools.

### `openrepro integrations run <project_name> [--target TARGET] [--confirm]`

Writes adapter execution plans and helper scripts, then optionally runs them
when `--confirm` is provided and the optional dependency is available.

```text
workspace/integration_execution.json
workspace/INTEGRATION_EXECUTION.md
integrations/*_execution_plan.json
integrations/run_*_adapter.py
```

Without `--confirm`, this is a dry-run plan. Missing optional dependencies are
recorded as skipped rather than treated as successful execution.

### `openrepro integrations summary <project_name>`

Shows integration export status and the latest adapter execution status.

### `openrepro promote plan <project_name> --target TARGET --candidate-id ID [--to STATE]`

Evaluates promotion gates and writes:
...
Targets are `experiment`, `report`, and `delivery`. States are `validated`,
`accepted`, `released`, and `rejected`.

### `openrepro promote record <project_name> --target TARGET --candidate-id ID [--to STATE] [--confirm]`

Writes a dry-run or confirmed promotion record:
...
Without `--confirm`, no registry entry is written. With `--confirm`, all
required gates must pass before the promotion is recorded.

### `openrepro promote summary <project_name>`

Shows the promotion registry and latest plan/record status.

### `openrepro github pr-summary <project_name> [--pr-number N] [--repo-dir PATH]`

Generates a local PR comment draft and writes:
...
The summary combines local CI scaffold validation, local UI, asset build,
plugin validation, promotion state, dashboard status, evidence freshness, and
local git metadata when available.

### `openrepro github summary <project_name>`
...
### `openrepro assets plan <project_name> [--target TARGET] [--refresh-catalog]`

Plans safe incremental asset materialization and writes:
...
The plan reads workflow state and the asset catalog, then marks safe runnable
steps with missing outputs as materialization candidates. Unsafe source input,
human decisions, repairs, and experiment execution stay blocked.

### `openrepro assets materialize <project_name> [--target TARGET] [--step STEP] [--confirm]`

Writes a dry-run or confirmed materialization record:
...
Without `--confirm`, no commands execute. With `--confirm`, materialization
delegates safe candidates to `openrepro workflow execute`.

### `openrepro assets summary <project_name>`

Shows the current asset build plan and latest materialization status.

### `openrepro cache add <project_name> [--path PATH]`

Adds standard project artifacts, or one specific file/directory, to the local
content-addressed cache and writes:

```text
workspace/artifact_cache.json
workspace/ARTIFACT_CACHE.md
.openrepro/cache/sha256/<prefix>/<sha256>
```

The cache stores observed bytes by SHA-256 for offline reuse and drift checks.
It is not a provenance guarantee or a scientific validation result.

### `openrepro cache list <project_name>`

Lists cached artifact records in the console.

### `openrepro cache verify <project_name>`

Verifies cached blobs against recorded hashes and writes:
...
The validation also reports changed source files while treating intact cached
blobs as valid.

### `openrepro cache gc <project_name>`
...
- v1.46.0: asset-centric incremental build plans and safe materialization.
...
- v1.56.0: static reproduction cockpit with next-action review surface.

See `ROADMAP.md` for details.

## Disclaimer

OpenRepro-Agent v1.56.0 is an engineering scaffold for reproducibility workflows. It should not be used to claim that a paper has been reproduced unless the user has independently verified formulas, parameters, code, data, and outputs.
