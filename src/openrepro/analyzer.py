"""Rule-based analyzer for OpenRepro-Agent v0.2.0.
...
def analyze_project(project_dir: Path) -> dict[str, Any]:
    ...
    analyzer_version = (config.get("analysis") or {}).get("analyzer_version", "v0.2.0-rule")
...
该账本由 OpenRepro-Agent v0.2.0 的规则分析器生成，用于保存论文复现中的模型候选、公式候选、参数候选和人工复核状态。所有候选默认标记为 `candidate_unverified`。
