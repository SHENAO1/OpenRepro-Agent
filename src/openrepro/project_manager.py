...
class ProjectStatus:
    ...
    delivery_bundle_exists: bool
    delivery_bundle_status: str
    delivery_bundle_required_file_count: int
    delivery_bundle_present_file_count: int
    delivery_bundle_missing_file_count: int
    delivery_bundle_top_command: str | None
    multi_agent_plan_exists: bool
    multi_agent_plan_status: str
    multi_agent_plan_agent_count: int
    multi_agent_plan_task_count: int
    ...
    agent_board_exists: bool
    agent_board_status: str
    agent_board_agent_count: int
    agent_board_task_count: int
    agent_board_open_task_count: int
    agent_board_human_input_task_count: int
    agent_board_top_command: str | None
    agent_dispatch_exists: bool
    agent_dispatch_status: str
    agent_dispatch_agent_count: int
    agent_dispatch_task_count: int
    agent_dispatch_open_task_count: int
    agent_dispatch_human_input_task_count: int
    agent_dispatch_top_command: str | None
    agent_exec_plan_exists: bool
    agent_exec_plan_status: str
    agent_exec_plan_dry_run: bool
...
def get_status(project_name: str | Path) -> ProjectStatus:
    ...
    if not exists:
        return ProjectStatus(
            ...
            delivery_bundle_exists=False,
            delivery_bundle_status="missing",
            delivery_bundle_required_file_count=0,
            delivery_bundle_present_file_count=0,
            delivery_bundle_missing_file_count=0,
            delivery_bundle_top_command=None,
            multi_agent_plan_exists=False,
            multi_agent_plan_status="missing",
            multi_agent_plan_agent_count=0,
            multi_agent_plan_task_count=0,
            ...
            agent_board_exists=False,
            agent_board_status="missing",
            agent_board_agent_count=0,
            agent_board_task_count=0,
            agent_board_open_task_count=0,
            agent_board_human_input_task_count=0,
            agent_board_top_command=None,
            agent_dispatch_exists=False,
            agent_dispatch_status="missing",
            agent_dispatch_agent_count=0,
            agent_dispatch_task_count=0,
            agent_dispatch_open_task_count=0,
            agent_dispatch_human_input_task_count=0,
            agent_dispatch_top_command=None,
            agent_exec_plan_exists=False,
            agent_exec_plan_status="missing",
            agent_exec_plan_dry_run=False,
    ...
    if not ingested:
        ...
        next_step = f"Run: {command}"
    elif not delivery_bundle["present"]:
        next_step = f"Run: openrepro delivery-bundle {project_dir} --zip"
    elif delivery_bundle["status"] != "ready":
        command = str(delivery_bundle["top_command"] or f"openrepro delivery-bundle {project_dir} --zip")
        next_step = f"Run: {command}"
    elif not multi_agent_plan["present"]:
        next_step = f"Run: openrepro multi-agent-plan {project_dir}"
        ...
        next_step = f"Run: {command}"
    elif not agent_board["present"]:
        next_step = f"Run: openrepro agent-board {project_dir} --zip"
    elif agent_board["status"] not in {"complete", "ready"}:
        command = str(agent_board["top_command"] or f"openrepro agent-board {project_dir} --zip")
        next_step = f"Run: {command}"
    elif not agent_dispatch["present"]:
        next_step = f"Run: openrepro agent-dispatch {project_dir}"
    elif agent_dispatch["status"] not in {"complete", "ready"}:
        command = str(agent_dispatch["top_command"] or f"openrepro agent-dispatch {project_dir}")
        next_step = f"Run: {command}"
    elif not agent_exec_plan["present"]:
    ...
    return ProjectStatus(
        ...
        delivery_bundle_exists=bool(delivery_bundle["present"]),
        delivery_bundle_status=str(delivery_bundle["status"]),
        delivery_bundle_required_file_count=int(delivery_bundle["required_file_count"] or 0),
        delivery_bundle_present_file_count=int(delivery_bundle["present_file_count"] or 0),
        delivery_bundle_missing_file_count=int(delivery_bundle["missing_file_count"] or 0),
        delivery_bundle_top_command=delivery_bundle["top_command"],
        multi_agent_plan_exists=bool(multi_agent_plan["present"]),
        multi_agent_plan_status=str(multi_agent_plan["status"]),
        multi_agent_plan_agent_count=int(multi_agent_plan["agent_count"] or 0),
        multi_agent_plan_task_count=int(multi_agent_plan["task_count"] or 0),
        ...
        agent_board_exists=bool(agent_board["present"]),
        agent_board_status=str(agent_board["status"]),
        agent_board_agent_count=int(agent_board["agent_count"] or 0),
        agent_board_task_count=int(agent_board["task_count"] or 0),
        agent_board_open_task_count=int(agent_board["open_task_count"] or 0),
        agent_board_human_input_task_count=int(agent_board["human_input_task_count"] or 0),
        agent_board_top_command=agent_board["top_command"],
        agent_dispatch_exists=bool(agent_dispatch["present"]),
        agent_dispatch_status=str(agent_dispatch["status"]),
        agent_dispatch_agent_count=int(agent_dispatch["agent_count"] or 0),
        agent_dispatch_task_count=int(agent_dispatch["task_count"] or 0),
        agent_dispatch_open_task_count=int(agent_dispatch["open_task_count"] or 0),
        agent_dispatch_human_input_task_count=int(agent_dispatch["human_input_task_count"] or 0),
        agent_dispatch_top_command=agent_dispatch["top_command"],
        agent_exec_plan_exists=bool(agent_exec_plan["present"]),
        agent_exec_plan_status=str(agent_exec_plan["status"]),
        agent_exec_plan_dry_run=bool(agent_exec_plan["dry_run"]),
